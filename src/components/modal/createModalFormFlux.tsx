...
import { Flux, FluxStep, TimeUnit } from "./modalFormFluxes";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { StepBuilder } from "./modalStepBuilderFluxes";
import { ArrowLeft, ArrowRight, Check, Sparkles, X, Clock, ChevronDown } from "lucide-react";
...
export function CreateFluxModal({
  ...
  // Estado para o novo sistema de Select
  ...
  return (
    ...
    >
      ...
      <div 
        className="relative z-[110] w-full max-w-md bg-white rounded-xl shadow-2xl flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 transition-colors z-[120]"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="p-8 overflow-y-auto">
          <div className="flex items-center gap-2 mb-8 pr-6">
            ...
            <div className="h-0.5 flex-1 bg-border relative">
              <div className={`absolute inset-y-0 left-0 bg-primary transition-all duration-500 ${wizardStep === "steps" ? "w-full" : "w-0"}`} />
            </div>
            <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold transition-all ${
          ...
          {wizardStep === "nickname" ? (
            <div className="space-y-8">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Defina o nome e a frequência do seu novo fluxo.
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="nickname" className="text-sm font-semibold text-gray-700">
                  Apelido do Fluxo <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="nickname"
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setIntervalValue(Math.max(1, val));
                      if (intervalError) setIntervalError("");
                    }}
                    className={`w-20 font-medium ${intervalError ? "border-destructive" : ""}`}
                  />
                  
                  {/* NOVO SISTEMA DE SELECT FIXED INSET-0 */}
                  <div className="relative flex-1">
                    <button
                      type="button"
                      onClick={() => setIsSelectOpen(true)}
                      className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-white px-3 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
                    >
                      <span className="truncate">
                        {intervalValue === 1 
                          ? TIME_UNIT_LABELS[intervalUnit].singular 
                          : TIME_UNIT_LABELS[intervalUnit].plural}
                      </span>
                      ...
                      {isSelectOpen && (
                        <>
                          <motion.div
                            ...
                            className="fixed inset-0 z-[200] bg-black/10 backdrop-blur-[1px]"
                          />
                          <div className="fixed inset-0 z-[201] flex items-center justify-center p-4 pointer-events-none">
                            <motion.div
                              ...
                              className="pointer-events-auto w-full max-w-[260px] bg-white rounded-xl shadow-2xl border border-border p-1 overflow-hidden"
                            >
                              <div className="px-3 py-2 text-[10px] font-bold text-muted-foreground uppercase tracking-wider opacity-60">
                                Unidade de Tempo
                              </div>
                              {(Object.keys(TIME_UNIT_LABELS) as TimeUnit[]).map((unit) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={cn(
                                      "flex w-full items-center justify-between px-3 py-2.5 text-sm transition-colors rounded-md",
                                      isSelected 
              ...
              <div className="flex justify-end pt-4 border-t">
                <Button onClick={handleNicknameNext} className="w-full sm:w-auto px-8 h-11 shadow-sm">
                  Próximo Passo
            ...
            <div className="space-y-6 flex flex-col h-full">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-bold">Construir Sequência</h2>
                  <p className="text-sm text-muted-foreground mt-1">
                    Fluxo: <span className="font-semibold text-primary">{nickname}</span>
                  </p>
                </div>
                <div className="px-3 py-1 bg-primary/10 rounded-full border border-primary/20 text-[11px] font-bold text-primary uppercase">
                  {intervalValue} {intervalValue === 1 ? TIME_UNIT_LABELS[intervalUnit].singular : TIME_UNIT_LABELS[intervalUnit].plural}
                </div>
              </div>
              ...
              <div className="flex justify-between pt-6 border-t gap-3 mt-auto bg-white">
                ...
                <Button onClick={handleSubmit} disabled={!canSubmit} className="flex-1 sm:flex-none px-8 h-11 shadow-md">
